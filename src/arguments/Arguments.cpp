...
Arguments::Arguments(const int& argc, const char* argv[]) {
	...
	std::unique_ptr<cxxopts::Options> allocated(new cxxopts::Options(argv[0], " - a simple HTTP client, made for educational purposes."));
	...
	for (auto& i : pos)
		this->_urls.push_back(i);
}

const std::string& Arguments::data() const {
	return this->_data;
}

const std::string& Arguments::httpMethod() const {
